...
public class OptionTemplateRecord extends Record {
	...
	public static OptionTemplateRecord parse(byte[] data) throws HeaderParseException {
		try {
			...
			otr.setTemplateID(UtilityManager.twoBytesToInteger(templateID));
			// field count
			byte[] fieldCount = new byte[2];
			System.arraycopy(data, 2, fieldCount, 0, 2);
			otr.setFieldCount(UtilityManager.twoBytesToInteger(fieldCount));
			// scope field count
			byte[] scopeFieldCount = new byte[2];
			System.arraycopy(data, 4, scopeFieldCount, 0, 2);
			otr.setScopeFieldCount(UtilityManager.twoBytesToInteger(scopeFieldCount));
			int offset = 6;
	...
	public byte[] getBytes() throws HeaderBytesException {
		try {
			...
			// template ID
			System.arraycopy(UtilityManager.integerToTwoBytes(getTemplateID()), 0, data, 0, 2);
			// field count
			System.arraycopy(UtilityManager.integerToTwoBytes(getFieldCount()), 0, data, 2, 2);
			// scope field count
			System.arraycopy(UtilityManager.integerToTwoBytes(getScopeFieldCount()), 0, data, 4, 2);
