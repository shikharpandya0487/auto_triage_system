...
public class SetHeader extends AbstractHeader implements IPFIXEntity {
	...
	public static SetHeader parse(byte[] data) throws HeaderParseException {
		try {
			...
			sh.setSetID(UtilityManager.twoBytesToInteger(setID));
			// length
			byte[] length = new byte[2];
			System.arraycopy(data, 2, length, 0, 2);
			sh.setLength(UtilityManager.twoBytesToInteger(length));
			// 2 -> template sets;
			if (sh.getSetID() == 2) {
				int offset = 4;
	...
	public byte[] getBytes() throws HeaderBytesException {
		try {
			...
			// set id
			System.arraycopy(UtilityManager.integerToTwoBytes(getSetID()), 0, data, 0, 2);
			// length
			System.arraycopy(UtilityManager.integerToTwoBytes(getLength()), 0, data, 2, 2);
			// data record
			int offset = 4;
